...
///
/// **The concurrency gate.** A refresh asks every source at once, plus a dozen
/// market instruments. Unbounded, that opens dozens of sockets and every
/// request gets slower than if they had simply queued.
actor HTTP {
    ...
    /// Feeds want a feed-first `Accept` header, so this exists rather than
