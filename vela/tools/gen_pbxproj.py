#!/usr/bin/env python3
"""Generate ios/Vela.xcodeproj from what is actually on disk.
...
Run: python3 vela/tools/gen_pbxproj.py [--check]
"""
...
def main():
    ...
    if check_only:
        ...
        if not os.path.exists(pbxproj_path):
            ...
            problems.append("project.pbxproj is stale — run vela/tools/gen_pbxproj.py")
        if not os.path.exists(scheme_path):
            problems.append("shared scheme is missing")
        elif open(scheme_path).read() != scheme:
            problems.append("shared scheme is stale — run vela/tools/gen_pbxproj.py")
