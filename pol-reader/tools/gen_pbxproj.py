#!/usr/bin/env python3
"""Generate ios/PolReader.xcodeproj from what is actually on disk.
...
Run: python3 tools/gen_pbxproj.py [--check]
"""
...
PROJECT_NAME = "PolReader"
BUNDLE_ID = "com.nurtrino.polreader"
...
def main():
    ...
    if check_only:
        ...
        if not os.path.exists(pbxproj_path):
            ...
            problems.append("project.pbxproj is stale — run tools/gen_pbxproj.py")
        if not os.path.exists(scheme_path):
            problems.append("shared scheme is missing")
        elif open(scheme_path).read() != scheme:
            problems.append("shared scheme is stale — run tools/gen_pbxproj.py")
